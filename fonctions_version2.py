         ...
         if liste[2]!="xml" and liste[2]!="json" :
           ...
           if liste[3]!="-t":
             ...
             if liste[4]!="-f" and liste[4]!="-h":
               ...
               if liste[4]=="-f" and ((liste[2]=="xml" and liste[5].find(".xml")==-1)or(liste[2]=="json" and liste[5].find(".json")==-1)):      
                  return 5
               else:
                 if liste[6]!="-o":
                   return 6
...
def creationGraphe(dwg,nElemnt):
  ...
  #Rectangle principal.
  dwg.add(dwg.rect(insert=(0, 0), size=(600*nElemnt, 280*nElemnt),fill='white', stroke='black'))
  
  i=0
  X1,Y1 = 200,125#Abscisse et ordonnée du rectangle.
  largr,longr=300,40
  XC,YC=700,Y1+longr*2#Abscisse et ordonnée du centre.
  XR,YR=110,70#Abscisse et ordonnée du rayon.
  X2,Y2=850,125#Abscisse et ordonnée du rectangle.
  while i<nElemnt:    
      #--Rectangle accueillant l'entité. 
      dwg.add(dwg.rect(insert=(X1, Y1), size=(largr, longr*4),fill=svgwrite.rgb(100,100,100,'%'), stroke=svgwrite.rgb(0,0,100,'%')))
      #Ligne séparant le nom de l'entité et ses attributs.
      hline = dwg.add(dwg.g(id='hline', stroke=svgwrite.rgb(0,0,100,'%')))
      hline.add(dwg.line(start=(X1,(Y1+30)),end=((largr+X1),(Y1+30))))
     
      #--Cercle accueillant l'association.
      dwg.add(dwg.ellipse(center=(XC, YC), r=(XR, YR),fill='white',stroke=svgwrite.rgb(0,0,100,'%')))
      #Ligne séparant l'association et ses attributs.
      hline = dwg.add(dwg.g(id='hline', stroke=svgwrite.rgb(0,0,100,'%')))
      hline.add(dwg.line(start=(XC-XR,YC),end=(XC+XR,YC)))
          
      #--Rectangle accueillant l'entité.
      dwg.add(dwg.rect(insert=(X2, Y2), size=(largr, longr*4),fill='white', stroke=svgwrite.rgb(0,0,100,'%')))
      #Ligne séparant le nom de l'entité et ses attributs.
      hline = dwg.add(dwg.g(id='hline', stroke=svgwrite.rgb(0,0,100,'%')))
      ...
      i+=1
  hline = dwg.add(dwg.g(id='hline', stroke=svgwrite.rgb(0,0,100,'%')))
  hline.add(dwg.line(start=(400,90),end=(1090,90),font_size=80))    
  return dwg               
...
def attributXML(dwg, nElemnt, Element_EA):
  ...
  #Texte entité 1
      ...
      dwg.add(dwg.text("NumCommande: "+Element_EA[i+1].attrib["NumCommande"],insert=(610,Y1+95),font_size=20))
      dwg.add(dwg.text("Date: "+Element_EA[i+1].attrib["date"],insert=(625,Y1+117),font_size=20))
      dwg.add(dwg.text("Quantite: "+Element_EA[i+1].attrib["quantite"],insert=(640,Y1+140),font_size=20))
...
def attributJSON(dwg, nElemnt, entite):                    
  ...
  #Texte entité 1
      ...
      dwg.add(dwg.text("NumCommande: "+entite[i][cles[1]]["NumCommande"],insert=(610,Y1+95),font_size=20))
      dwg.add(dwg.text("Date: "+entite[i][cles[1]]["date"],insert=(625,Y1+117),font_size=20))
      dwg.add(dwg.text("Quantite: "+entite[i][cles[1]]["quantite"],insert=(640,Y1+140),font_size=20))
